...
use common::event::{Event, RegionFlags};
...
fn test_compact_name() {
    ...
    assert_eq!(
        CompactName::new(0x1234FF, MapKind::Mmap).kind(),
        MapKind::Mmap
    );
    assert_eq!(
        CompactName::new(0x1234FF, MapKind::Mmap).kind().as_str(),
        "mmap"
    );
    assert_eq!(CompactName::new(0x1234FF, MapKind::Jemalloc).id(), 0x1234FF);
    assert_eq!(
        CompactName::new(0x1234FF, MapKind::Jemalloc).kind(),
        MapKind::Jemalloc
    );
    assert_eq!(
        CompactName::new(0x1234FF, MapKind::Jemalloc)
            .kind()
            .as_str(),
        "jemalloc"
    );
...
impl MapsRegistry {
    ...
    fn set_vma_name_slow(
        &mut self,
        pointer: *const libc::c_void,
        length: usize,
        name: CompactName,
    ) {
        self.emulated_vma_name_map
            .insert(pointer as u64..pointer as u64 + length as u64, name);
    }

    pub fn clear_vma_name(&mut self, pointer: *mut libc::c_void, length: usize) {
        if !crate::global::is_pr_set_vma_anon_name_supported() {
            self.emulated_vma_name_map
                .remove(pointer as u64..pointer as u64 + length as u64);
        }
    }
    ...
    pub fn on_mmap(
        ...
        for (range_unmapped, original_bucket) in self.mmap_by_address.remove(range.clone()) {
            ...
            for existing_unmap in existing_unmaps {
                trace!(
                    "On munmap through fixed mmap: {:016X}..{:016X}, old_id = {}, new_id = {}",
                    start,
                    existing_unmap.start,
                    original_bucket.id,
                    id
                );
                self.munmap_by_address
                    .insert(start..existing_unmap.start, bucket.clone());
            ...
            self.munmap_by_address
                .insert(start..range_unmapped.end, bucket.clone());
        }
        ...
        trace!(
            "On mmap: 0x{:016X}..0x{:016X}, id = {}, pages = {}",
            range.start,
            range.end,
            id,
            (range.end - range.start) / 4096
        );
    ...
    pub fn on_munmap(&mut self, range: Range<u64>, source: MapSource) {
        trace!(
            "On mummap: 0x{:016X}..0x{:016X}, pages = {}",
            range.start,
            range.end,
            (range.end - range.start) / 4096
        );
        ...
        for (removed_range, bucket) in self.mmap_by_address.remove(range) {
            trace!(
                "  Removed chunk: 0x{:016X}..0x{:016X}, id = {}, pages = {}",
                removed_range.start,
                removed_range.end,
                bucket.id,
                (removed_range.end - removed_range.start) / 4096
            );
            self.munmap_by_address.insert(
                removed_range,
                MapBucket {
                    id: bucket.id,
                    source: source.clone(),
...
fn emit_mmap(mmap: Mmap, backtrace_cache: &mut BacktraceCache, serializer: &mut impl Write) {
    let backtrace = crate::writers::write_backtrace(
        &mut *serializer,
        mmap.source.backtrace.clone(),
        backtrace_cache,
    )
    .ok()
    .unwrap_or(0);
...
fn emit_remove_region(
    ...
    let sources_out: smallvec::SmallVec<[common::event::RegionTargetedSource; 1]> = sources
        .into_iter()
        .map(|source| {
            let backtrace = crate::writers::write_backtrace(
                &mut *serializer,
                source.source.backtrace.clone(),
                backtrace_cache,
            )
            .ok()
            .unwrap_or(0);
...
fn emit_events(
    backtrace_cache: &mut BacktraceCache,
    serializer: &mut impl Write,
    new_events: impl IntoIterator<Item = PendingEvent>,
) {
    for event in new_events {
        match event {
            PendingEvent::Mmap { mmap, .. } => {
                emit_mmap(mmap, backtrace_cache, serializer);
            }
            PendingEvent::AddRegion {
                timestamp,
                id,
                ref info,
                flags,
                ...
                emit_add_region(timestamp, id, info, flags, name.as_str(), serializer);
            }
            PendingEvent::UpdateUsage {
                id,
                timestamp,
                address,
                length,
                ...
                emit_usage(id, address, length, timestamp, usage, serializer);
            }
            PendingEvent::RemoveRegion {
                timestamp,
                id,
                address,
                length,
...
fn generate_unmaps(
    ...
    for (unmap_range, unmap_bucket) in
        tmp_munmap_by_address.get_in_range(address_start..address_end)
    {
        trace!(
            "Found a source for an unmap: 0x{:016X}..0x{:016X}, id = {}, pages = {}",
            unmap_range.start,
            unmap_range.end,
            id,
            (unmap_range.end - unmap_range.start) / 4096
        );
...
pub fn update_smaps(
    ...
    let timestamp;
    {
        ...
        maps_registry
            .mmap_by_address
            .clone_into(&mut state.tmp_mmap_by_address);
        std::mem::swap(
            &mut maps_registry.munmap_by_address,
            &mut state.tmp_munmap_by_address,
        );
        ...
        maps_registry
            .emulated_vma_name_map
            .clone_into(&mut state.tmp_emulated_vma_name_map);

        timestamp = crate::timestamp::get_timestamp();
        let mut fp = std::fs::File::open("/proc/self/smaps").expect("failed to open smaps");
        fp.read_to_end(&mut state.tmp_buffer)
            .expect("failed to read smaps");
    ...
    let region_info_to_id: HashMap<RegionInfo, (u64, usize)> = state
        .map_by_id
        .iter()
        .flat_map(|(id, map)| {
            std::iter::once(id)
                .cycle()
                .zip(map.regions.iter().enumerate())
        })
    ...
    loop {
        ...
        let is_readable = if get_char(&mut line).unwrap() == 'r' {
            RegionFlags::READABLE
        } else {
            RegionFlags::empty()
        };
        let is_writable = if get_char(&mut line).unwrap() == 'w' {
            RegionFlags::WRITABLE
        } else {
            RegionFlags::empty()
        };
        let is_executable = if get_char(&mut line).unwrap() == 'x' {
            RegionFlags::EXECUTABLE
        } else {
            RegionFlags::empty()
        };
        let is_shared = if get_char(&mut line).unwrap() == 's' {
            RegionFlags::SHARED
        } else {
            RegionFlags::empty()
        };
        ...
        if !crate::global::is_pr_set_vma_anon_name_supported() {
            if name.starts_with(BYTEHOUND_MEMFD_PADDING) {
                state
                    .tmp_padding_maps
                    .push((address, address_end - address));
                is_padding = true;
            } else {
        ...
        if crate::global::is_pr_set_vma_anon_name_supported() && name.starts_with("[anon:") {
            ...
            let index_2 = name[index_1..]
                .find(" ")
                .map(|index_2| index_1 + index_2)
                .unwrap_or(name.len());
        ...
        match state.map_by_id.remove(&id) {
            Some(mut map) => {
                ...
                for new_region in new_regions {
                    if let Some(old_region_index) = map.regions.iter().position(|old_region| {
                        old_region.info == new_region.info && old_region.name == new_region.name
                    }) {
                        ...
                        trace!(
                            ...
                            state
                                .tmp_mmap_by_address
                                .get_value(new_region.info.address)
                                .is_some()
                        );
            ...
            None => {
                ...
                state.tmp_new_map_by_id.insert(
                    id,
                    Map {
                        regions: new_regions,
                    },
    ...
    state
        .tmp_all_new_events
        .sort_unstable_by_key(|event| match event {
            PendingEvent::Mmap {
                epoch,
                mmap: Mmap { id, address, .. },
                ..
            } => (*epoch, 1, *id, *address),
            PendingEvent::AddRegion {
                epoch,
                id,
                info: RegionInfo { address, .. },
                ..
            } => (*epoch, 2, *id, *address),
            PendingEvent::UpdateUsage {
                epoch, id, address, ..
    ...
    emit_events(
        backtrace_cache,
        serializer,
        state.tmp_all_new_events.drain(..),
    );
    std::mem::swap(&mut state.map_by_id, &mut state.tmp_new_map_by_id);

    for (address, length) in state.tmp_padding_maps.drain(..) {
        if state.tmp_seen_maps.contains(&(address - 1))
            || state.tmp_seen_maps.contains(&(address + length))
        {
            continue;
        }

        trace!(
            "Unmapping a padding map at 0x{:016X}..0x{:016X}",
            address,
            address + length
        );
        unsafe {
    ...
    for (id, mmap) in state.tmp_mmaps.drain() {
        debug!(
            "Map registered yet not found in smaps: 0x{:016X}..0x{:016X}, id = {}",
            mmap.address,
            mmap.address + mmap.requested_length,
            id
        );
    }
}
