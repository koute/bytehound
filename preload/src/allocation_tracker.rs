...
impl AllocationBucket {
    fn is_long_lived(&self, now: Timestamp) -> bool {
        now.as_usecs()
            >= self.events[0].timestamp.as_usecs()
                + crate::opt::get().temporary_allocation_lifetime_threshold * 1000
    }
}
...
struct AllocationTrackerRegistry {
    ...
    global:
        Mutex<crate::ordered_map::OrderedMap<(u64, u64), AllocationBucket, crate::nohash::NoHash>>,
}
...
pub fn initialize() {
    ENABLED.store(
        crate::opt::get().cull_temporary_allocations,
        Ordering::SeqCst,
    );
...
pub fn on_thread_created(unique_tid: u64) -> AllocationTracker {
    ...
    ALLOCATION_TRACKER_REGISTRY
        .write()
        .per_thread
        .insert(unique_tid, AllocationTracker(tracker.0.clone()));
    tracker
}
...
fn flush_pending<K>(
    ...
    K: Copy + PartialEq + Eq + std::hash::Hash,
{
    let temporary_allocation_pending_threshold = crate::opt::get()
        .temporary_allocation_pending_threshold
        .unwrap_or(!0);
    while let Some(key) = allocations.front_key() {
        let should_flush = allocations.len() > temporary_allocation_pending_threshold
...
pub fn on_allocation(
    ...
    if thread.is_dead() {
        ...
        zombie_events.push(InternalEvent::Alloc {
            id,
            timestamp,
            allocation,
            backtrace,
        ...
        bucket.events.push(BufferedAllocation {
            timestamp,
            allocation,
            backtrace,
        });
        ...
        if tracker.buckets.insert(id.allocation, bucket).is_some() {
            error!(
                "Duplicate allocation 0x{:08X} with ID {}; this should never happen",
                address, id
            );
        }
    ...
    crate::event::send_event_throttled_sharded(get_shard_key(id), move || InternalEvent::Alloc {
        id,
        timestamp,
        allocation,
        backtrace,
...
pub fn on_reallocation(
    ...
    if thread.is_dead() {
        let mut zombie_events = thread.zombie_events().lock();
        zombie_events.push(InternalEvent::Realloc {
            id,
            timestamp,
            old_address,
            allocation,
    ...
    crate::event::send_event_throttled_sharded(get_shard_key(id), move || InternalEvent::Realloc {
        id,
        timestamp,
        old_address,
        allocation,
...
pub fn on_free(
    ...
    if thread.is_dead() {
        let mut zombie_events = thread.zombie_events().lock();
        zombie_events.push(InternalEvent::Free {
            timestamp,
            id,
            address,
            backtrace,
        ...
        return;
    } else if !thread.is_dead()
        && ENABLED.load(Ordering::Relaxed)
        && !id.is_untracked()
        && !id.is_invalid()
    {
        let bucket = if id.thread == thread.unique_tid() {
            let mut tracker = thread.allocation_tracker().0.lock();
            flush_pending(&mut tracker.buckets, timestamp);
    ...
    crate::event::send_event_throttled_sharded(get_shard_key(id), move || InternalEvent::Free {
        timestamp,
        id,
        address,
        backtrace,
