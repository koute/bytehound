...
fn write_backtrace< U: Write >( serializer: &mut U, thread: u32, backtrace: Backtrace ) -> io::Result< u64 > {
    if backtrace.is_empty() {
        return Ok( 0 );
    }

    static LAST_BACKTRACE_ID: SpinLock< u64 > = spin_lock_new!( 1 );

    let id = {
        let mut guard = LAST_BACKTRACE_ID.lock();
        *guard += 1;
        *guard
    };
