...
impl Client {
    fn new(
        ...
        Response::Start(broadcast_header(id, initial_timestamp, listener_port))
            .write_to_stream(&mut client.stream)?;
        Ok(client)
    }
    ...
    pub(crate) fn assign_id(&mut self, backtrace: &Backtrace) -> (u64, bool) {
        ...
        match self.cache.get_mut(&key) {
            ...
            Some(cached_backtrace) => {
                if Backtrace::ptr_eq(&cached_backtrace, &backtrace)
                    || cached_backtrace.frames() == backtrace.frames()
                {
                    (
                        cached_backtrace
                            .id()
                            .expect("internal error: id was not set on a cached backtrace"),
                        false,
                    )
                } else {
...
fn emit_allocation_bucket(
    mut bucket: AllocationBucket,
...
pub(crate) fn thread_main() {
    ...
    loop {
        ...
        for event in events.drain() {
            match event {
                InternalEvent::Alloc {
                    ...
                    if let Ok(backtrace) =
                        writers::write_backtrace(&mut *serializer, backtrace, &mut backtrace_cache)
                    {
                        let _ = Event::AllocEx {
                            id,
                            timestamp,
                ...
                InternalEvent::AddressSpaceUpdated {
                    timestamp,
                    maps,
                    new_binaries,
                } => {
        ...
        coarse_timestamp = get_timestamp();
        let should_update_smaps = opt::get().gather_maps
            && (force_smaps_update || (coarse_timestamp - last_smaps_update).as_msecs() >= 1000);
