use crate::{Allocation, BacktraceId, Data, DataPointer, Map, MapId, Timestamp};
...
fn compile_backtrace_filter(
    data: &Data,
    filter: &RawBacktraceFilter,
) -> Option<HashSet<BacktraceId>> {
    let is_none = filter.only_passing_through_function.is_none()
    ...
    for (backtrace_id, backtrace) in data.all_backtraces() {
        ...
        let mut positive_matched = filter.only_passing_through_function.is_none()
            && filter.only_passing_through_source.is_none();
        let mut negative_matched = false;
        let check_negative = filter.only_not_passing_through_function.is_some()
        ...
        for (frame_id, frame) in backtrace {
            ...
            if (check_positive && filter.only_passing_through_function.is_some())
            ...
            let mut source = None;
            if (check_positive && filter.only_passing_through_source.is_some())
                || filter.only_not_passing_through_source.is_some()
            {
                source = frame
                    .source()
    ...
    if let Some(ref only_matching_backtraces) = filter.only_matching_backtraces {
        matched_backtraces = matched_backtraces
            .intersection(&only_matching_backtraces)
            .copied()
            .collect();
    }
...
impl Compile for RawCommonFilter {
    ...
    fn compile(&self, data: &Data) -> Self::Compiled {
        ...
        if self.only_deallocated_after_at_least.is_some()
            || self.only_deallocated_until_at_most.is_some()
        {
            only_deallocated_between_inclusive = Some((
                self.only_deallocated_after_at_least
                    .map(|offset| data.initial_timestamp + offset.0)
                    .unwrap_or(data.initial_timestamp),
        ...
        let mut only_leaked_or_deallocated_after = self
        ...
        if self.only_temporary {
            if let Some((ref mut min, ref mut max)) = only_deallocated_between_inclusive {
                ...
                *max = std::cmp::min(*max, data.last_timestamp);
                ...
                only_deallocated_between_inclusive =
                    Some((data.initial_timestamp, data.last_timestamp));
            }
        }

        let only_allocated_after_at_least = self
            .only_allocated_after_at_least
            .map(|offset| data.initial_timestamp + offset.0)
            .unwrap_or(data.initial_timestamp);
        let mut only_allocated_until_at_most = self
        ...
        for only_alive_at in &self.only_alive_at {
            ...
            only_allocated_until_at_most =
                std::cmp::min(only_allocated_until_at_most, only_alive_at);
            only_leaked_or_deallocated_after =
                std::cmp::max(only_leaked_or_deallocated_after, only_alive_at);
        }
...
impl Compile for RawAllocationFilter {
    ...
    fn compile(&self, data: &Data) -> Self::Compiled {
        ...
        if let Some(only_first_size_larger) = self.only_first_size_larger {
            if only_first_size_larger == !0 {
                ...
                only_first_size_larger_or_equal =
                    std::cmp::max(only_first_size_larger_or_equal, only_first_size_larger + 1);
            }
        }

        let mut only_first_size_smaller_or_equal =
            self.only_first_size_smaller_or_equal.unwrap_or(!0);
        if let Some(only_first_size_smaller) = self.only_first_size_smaller {
            if only_first_size_smaller == 0 {
                is_impossible = true;
            } else {
                only_first_size_smaller_or_equal = std::cmp::min(
                    only_first_size_smaller_or_equal,
                    only_first_size_smaller - 1,
                );
            }
        }
        ...
        if let Some(only_last_size_larger) = self.only_last_size_larger {
            if only_last_size_larger == !0 {
                ...
                only_last_size_larger_or_equal =
                    std::cmp::max(only_last_size_larger_or_equal, only_last_size_larger + 1);
            }
        }

        let mut only_last_size_smaller_or_equal =
            self.only_last_size_smaller_or_equal.unwrap_or(!0);
        if let Some(only_last_size_smaller) = self.only_last_size_smaller {
            if only_last_size_smaller == 0 {
                is_impossible = true;
            } else {
                only_last_size_smaller_or_equal =
                    std::cmp::min(only_last_size_smaller_or_equal, only_last_size_smaller - 1);
            }
        }
        ...
        if let Some(ref map_ids) = self.only_from_maps {
            if map_ids.is_empty() {
                        ...
                        map.deallocation
                            ...
                            .map(|deallocation| {
                                deallocation
                                    ...
                                    .as_ref()
                                    .map(|source| source.timestamp)
                                    .unwrap_or(deallocation.timestamp)
                            })
                            .unwrap_or(data.last_timestamp),
                    )
                });

                let (mut address_min, mut address_max, mut timestamp_min, mut timestamp_max) =
                    iter.next().unwrap();
                ...
                common_filter.only_address_at_least =
                    std::cmp::max(common_filter.only_address_at_least, address_min);
                common_filter.only_address_at_most =
                    std::cmp::min(common_filter.only_address_at_most, address_max);
                common_filter.only_allocated_after_at_least =
                    std::cmp::max(common_filter.only_allocated_after_at_least, timestamp_min);
                common_filter.only_allocated_until_at_most =
                    std::cmp::min(common_filter.only_allocated_until_at_most, timestamp_max);
            }
        }

        let enable_chain_filter = self.only_first_size_larger.is_some()
        ...
        let enable_group_filter = self.only_group_allocations_at_least.is_some()
        ...
        RawCompiledAllocationFilter {
            ...
            backtrace_filter: RawCompiledBacktraceFilter {
                only_backtraces,
                only_not_matching_backtraces: self
                    .backtrace_filter
                    .only_not_matching_backtraces
                    .clone(),

                only_deallocation_backtraces: self
                    .backtrace_filter
                    .only_matching_deallocation_backtraces
                    .clone(),
                only_not_matching_deallocation_backtraces: self
                    .backtrace_filter
            ...
            only_chain_alive_for_at_least: self
            ...
            only_group_interval_at_least: self
                .only_group_interval_at_least
                .unwrap_or(Duration::from_secs(0)),
            only_group_interval_at_most: self
                .only_group_interval_at_most
                .unwrap_or(Duration::from_secs(5000 * 365 * 24 * 3600)),
            only_group_max_total_usage_first_seen_at_least: self
                .only_group_max_total_usage_first_seen_at_least
                .map(|offset| data.initial_timestamp + offset.0)
                .unwrap_or(data.initial_timestamp),
            only_group_max_total_usage_first_seen_at_most: self
                .only_group_max_total_usage_first_seen_at_most
                .map(|offset| data.initial_timestamp + offset.0)
                .unwrap_or(data.last_timestamp),
            only_group_leaked_allocations_at_least: self
                .only_group_leaked_allocations_at_least
                .unwrap_or(NumberOrFractionOfTotal::Number(0)),
            only_group_leaked_allocations_at_most: self
                .only_group_leaked_allocations_at_most
                .unwrap_or(NumberOrFractionOfTotal::Number(!0)),

            enable_group_filter,

            only_ptmalloc_mmaped: if self.only_ptmalloc_mmaped {
                Some(true)
            ...
            only_ptmalloc_from_main_arena: if self.only_ptmalloc_from_main_arena {
                Some(true)
            ...
            only_jemalloc: if self.only_jemalloc {
                Some(true)
            ...
            only_from_maps: self
                .only_from_maps
                .as_ref()
                .map(|only_from_maps| only_from_maps.iter().copied().collect()),
        }
    }
}
...
impl Compile for RawMapFilter {
    ...
    fn compile(&self, data: &Data) -> Self::Compiled {
        let mut is_impossible = (self.only_bytehound && self.only_not_bytehound)
...
impl TryMatch for RawCompiledCommonFilter {
    ...
    fn try_match(&self, data: &Data, allocation: &Self::Item) -> bool {
        ...
        if !(allocation.size <= self.only_smaller_or_equal
            && allocation.size >= self.only_larger_or_equal)
        {
            return false;
        }

        if !(allocation.pointer >= self.only_address_at_least
            && allocation.pointer <= self.only_address_at_most)
        {
            return false;
        }

        if !(allocation.timestamp >= self.only_allocated_after_at_least
            && allocation.timestamp <= self.only_allocated_until_at_most)
        {
            return false;
        }
        ...
        let lifetime_end = allocation
            .deallocation_timestamp
            .unwrap_or(data.last_timestamp());
...
impl TryMatch for RawCompiledAllocationFilter {
    ...
    fn try_match(&self, data: &Data, allocation: &Allocation) -> bool {
        ...
        if !self.common_filter.try_match(
            data,
            &CommonFilterArgs {
                pointer: allocation.pointer,
                size: allocation.size,
                timestamp: allocation.timestamp,
                deallocation_timestamp: allocation
                    .deallocation
                    .as_ref()
        ...
        if !self.backtrace_filter.try_match(
            data,
            &BacktraceFilterArgs {
                backtrace: Some(allocation.backtrace),
                deallocation_backtrace: allocation
                    .deallocation
                    .as_ref()
                    .map(|deallocation| deallocation.backtrace),
            },
        ) {
            return false;
        }
        ...
        if self.enable_chain_filter {
            ...
            if let Some(first_in_chain) = allocation.first_allocation_in_chain {
                ...
                chain_lifetime_end = last_allocation
                    .deallocation
                    .as_ref()
                    .map(|deallocation| deallocation.timestamp)
                    .unwrap_or(data.last_timestamp());
            ...
            if !(first_allocation_size <= self.only_first_size_smaller_or_equal
                && first_allocation_size >= self.only_first_size_larger_or_equal)
            {
                return false;
            }

            if !(last_allocation_size <= self.only_last_size_smaller_or_equal
                && last_allocation_size >= self.only_last_size_larger_or_equal)
            {
                return false;
            }

            if !(chain_length >= self.only_chain_length_at_least
                && chain_length <= self.only_chain_length_at_most)
            {
                return false;
            }
            ...
            if !(allocation.position_in_chain >= self.only_position_in_chain_at_least
                && allocation.position_in_chain <= self.only_position_in_chain_at_most)
            {
                return false;
            }
        }
        ...
        if self.enable_group_filter {
            ...
            if stats.max_total_usage_first_seen_at
        ...
        if let Some(ref only_from_maps) = self.only_from_maps {
            if !only_from_maps
                .iter()
                .any(|&map_id| data.maps[map_id.raw() as usize].try_match_allocation(allocation))
            {
                return false;
            }
        }
    ...
    fn try_match(&self, data: &Data, map: &Map) -> bool {
        ...
        if !self.backtrace_filter.try_match(
            ...
            &BacktraceFilterArgs {
                ...
                deallocation_backtrace: map
                    .deallocation
                    .as_ref()
                    .map(|deallocation| deallocation.source.map(|source| source.backtrace)),
            },
        ) {
            return false;
        }
    ...
    fn try_match(&self, data: &Data, allocation: &Self::Item) -> bool {
        match *self {
            ...
            Self::And(ref lhs, ref rhs) => {
                lhs.try_match(data, allocation) && rhs.try_match(data, allocation)
            }
    ...
    fn compile(&self, data: &Data) -> Self::Compiled {
        match *self {
            ...
            Filter::And(ref lhs, ref rhs) => {
                Filter::And(Box::new(lhs.compile(data)), Box::new(rhs.compile(data)))
            }
            Filter::Or(ref lhs, ref rhs) => {
                Filter::Or(Box::new(lhs.compile(data)), Box::new(rhs.compile(data)))
            }
