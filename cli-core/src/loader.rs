...
use crate::data::{
    Allocation, AllocationChain, AllocationFlags, AllocationId, BacktraceId, BacktraceStorageRef,
    CodePointer, Data, DataId, DataPointer, Deallocation, FrameId, GroupStatistics, Mallopt, Map,
    MapDeallocation, MapId, MapRegion, MapRegionDeallocation, MapRegionDeallocationSource,
    MapSource, MapUsage, OperationId, RegionFlags, StringId, StringInterner, ThreadId, Timestamp,
    UsageDelta,
};
        ...
        F: FnMut(FrameId, bool),
    {
        match event {
            ...
            Event::Backtrace32 {
                ...
                raw_id,
                addresses.iter().map(|&p| p as u64).collect(),
                callback,
            ),
            _ => {
                unreachable!();
    ...
    pub fn process(&mut self, event: Event) {
        match event {
            ...
            Event::MemoryMapEx {
                ...
                self.delta_list_for_map.insert(
                    id,
                    vec![(
                        source.timestamp,
                        UsageDelta {
            ...
            Event::AddRegion {
                ...
                trace!(
                    "Add region: id = {} ({}), address = 0x{:016X}, length = {}, source = {}",
                    self.map_index_map.get(&map_id).unwrap().raw(),
                    map_id,
                    address,
                    length,
                    map.source.is_some()
                ...
                self.last_usage_for_region
                    .insert((map.id, map.regions.len()), usage);
            ...
            Event::RemoveRegion {
                ...
                let id = match self.map_index_map.get(&map_id) {
                    ...
                    None => {
                        warn!(
                            "Remove map for map which was not found: id = {}, address = 0x{:016X}",
                            map_id, address
                        );
                        return;
                    }
                ...
                let (region_index, region) = match map
                    .regions
                    .iter_mut()
                    .enumerate()
                    .rev()
                    .find(|(_, region)| region.pointer == address && region.size == length)
                {
                    Some(region) => region,
                    None => {
                ...
                trace!(
                    "Remove region: id = {} ({}), address = 0x{:016X}, length = {}",
                    id.0,
                    map_id,
                    address,
                    length
                ...
                if region.size != length {
                    warn!(
                        "Length doesn't match for region at 0x{:016X} region removal",
                        address
                    );
                }

                region.deallocation = Some(MapRegionDeallocation { timestamp, sources });

                let last_usage = self
                    .last_usage_for_region
                    .remove(&(map.id, region_index))
                    .unwrap();
            ...
            Event::UpdateRegionUsage {
                ...
                let id = match self.map_index_map.get(&map_id) {
                    ...
                    None => {
                        warn!(
                            "Update for map which was not found: id = {}, address = 0x{:016X}",
                            map_id, address
                        );
                        return;
                    }
                ...
                let (region_index, region) = match map
                    ...
                    .enumerate()
                    .rev()
                    .find(|(_, region)| region.pointer == address)
                {
                ...
                if region.size != length {
                    warn!(
                        "Length doesn't match for region at 0x{:016X} for region update",
                        address
                    );
                    return;
                }
    ...
    pub fn finalize(mut self) -> Data {
        ...
        for &op in &operations {
            ...
            if current > current_total_max_size_by_backtrace[backtrace.raw() as usize].0 {
                current_total_max_size_by_backtrace[backtrace.raw() as usize] =
        ...
        for (backtrace_id, mut allocation_ids) in index {
            debug_assert!(
                allocation_ids.is_empty()
                    || allocations[allocation_ids[0].raw() as usize].backtrace == backtrace_id
            );
            ...
            if map.regions.len() == 1 {
                map.deallocation =
                    map.regions[0]
                        .deallocation
                        .as_ref()
                        .map(|deallocation| MapDeallocation {
                ...
                if map
                    .regions
                    .iter()
                    .all(|region| region.deallocation.is_some())
                {
                    let deallocation = map
                        .regions
                        .iter()
                        .map(|region| region.deallocation.as_ref().unwrap())
                        .max_by_key(|deallocation| deallocation.timestamp)
                        .unwrap();
