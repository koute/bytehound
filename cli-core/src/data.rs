...
impl AllocationId {
    ...
    pub(crate) fn new(raw: u64) -> Self {
        unsafe { AllocationId(NonZeroU64::new_unchecked(raw + 1)) }
    }
...
impl Map {
    ...
    pub fn try_match_allocation(&self, allocation: &Allocation) -> bool {
        let map_timestamp = self
            .source
            .as_ref()
            .map(|source| source.timestamp)
            .unwrap_or(self.timestamp);
        let map_timestamp_end = self.deallocation.as_ref().map(|deallocation| {
            deallocation
                .source
                .as_ref()
                .map(|source| source.timestamp)
        ...
        if allocation.timestamp < map_timestamp
            || map_timestamp_end
                .map(|map_timestamp_end| allocation.timestamp >= map_timestamp_end)
                .unwrap_or(false)
        {
            return false;
        }
            ...
            for region in &self.regions {
                if let Some(deallocation) = region.deallocation.as_ref() {
                    for source in &deallocation.sources {
                        if source.address <= allocation.pointer
...
pub trait SliceLikeIterator:
    DoubleEndedIterator + ExactSizeIterator + FusedIterator + Clone
{
}

impl<T> SliceLikeIterator for T where
    T: DoubleEndedIterator + ExactSizeIterator + FusedIterator + Clone
{
}
...
impl Data {
    ...
    pub fn all_backtraces<'a>(
        ...
        Item = (
            BacktraceId,
            impl SliceLikeIterator<Item = (FrameId, &'a Frame)>,
        ),
    > {
