use crate::Timestamp;
use crate::{Data, Map, MapUsage, OperationId, UsageDelta};
...
pub trait Delta:
    Copy + Clone + Default + std::ops::Add<Output = Self> + std::ops::Sub<Output = Self>
{
...
pub fn build_map_timeline(
    ...
    build_timeline(timestamp_min, timestamp_max, 1000, ops.iter().copied())
}
    ...
    T: Delta,
{
    let granularity = std::cmp::max(
        (timestamp_max - timestamp_min).as_usecs() / point_count as u64,
        1,
    );
...
fn test_build_timeline_one_point() {
    let output = build_timeline::<i64>(
        ...
        Timestamp::from_usecs(10000),
        100,
        vec![(Timestamp::from_usecs(10000), 100)].into_iter(),
    );
